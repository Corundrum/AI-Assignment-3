...
bool Agent::checkAgentLOSToTarget(Agent * agent, DisplayObject * target_obstacle, std::vector<Obstacle*>&obstacles)
{
	bool hasLOS = false;
	...
	auto AgentToTargetDistance = Util::getClosestEdge(agent->getTransform()->position, target_obstacle);
